  ...
  for csv_name in csv_filenames:
    ...
    if len(cols)>2:
      ...
      if len(saving_df)==0:
        ...
        alltimes = np.arange(1, max(dftimes)+1., dtype='float')
        if len(list(set(alltimes)-set(dftimes)))==0:
          continue
        else:
          # add a row for neutral frame by choosing a random neutral frame
          random_neutral_time = np.random.choice(list(set(alltimes)-set(dftimes)))
      ...
      discardCount += 1
  file.close()
  print('Formatted and saved a total of {} files, they are available in {}'.format(saveCount,save_dir))
