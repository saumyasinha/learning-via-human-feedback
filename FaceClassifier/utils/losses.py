...
def focal_loss(y_true, y_pred, alpha=0.25, gamma=2.0):
    ...
    loss = -alpha * K.pow(1.0 - y_pred, gamma) * y_true * K.log(y_pred)
...
def binary_focal_loss(y_true, y_pred, gamma=2.0, alpha=0.25):
    ...
    return -K.mean(alpha * K.pow(1.0 - pt_1, gamma) * K.log(pt_1), axis=-1) - K.mean(
        (1 - alpha) * K.pow(pt_0, gamma) * K.log(1.0 - pt_0), axis=-1
    )
