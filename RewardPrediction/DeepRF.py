...
# Convolutional Net with dropout, equivalent to self.conv in DeepNeuralDecisionForest
# with softmax for predictions
class Net(nn.Module):
    def __init__(self):
...
class DeepNeuralDecisionForest(nn.Module):
    def __init__(self, p_keep_conv, p_keep_hidden, n_leaf, n_label, n_tree, n_depth):
        ...
        for i in range(self._ntree):
            ...
            self.pi_e.append(
                Parameter(self.init_prob_weights([self._nleaf, self._nlabel], -2, 2))
            )
    ...
    def compute_mu(self, flat_decision_p_e):
        ...
        batch_0_indices = (
            torch.arange(0, n_batch * self._nleaf, self._nleaf)
            .unsqueeze(1)
            .repeat(1, self._nleaf)
            .long()
        )
        ...
        batch_complement_indices = torch.LongTensor(
            np.array(
                [[0] * in_repeat, [n_batch * self._nleaf] * in_repeat] * out_repeat
            ).reshape(n_batch, self._nleaf)
        )

        # First define the routing probabilistics d for root nodes
        mu_e = []
        indices_var = (batch_0_indices + batch_complement_indices).view(-1)
        if args.cuda:
            indices_var = indices_var.cuda()

        # iterate over each tree
        for i, flat_decision_p in enumerate(flat_decision_p_e):
            mu = torch.gather(flat_decision_p, 0, indices_var).view(
                n_batch, self._nleaf
            )
            mu_e.append(mu)
        ...
        for d in range(1, self._ndepth + 1):
            ...
            tile_indices = (
                indices.unsqueeze(1).repeat(1, 2 ** (self._ndepth - d + 1)).view(1, -1)
            )
            ...
            mu_e_update = []
            indices_var = (batch_indices + batch_complement_indices).view(-1)
            if args.cuda:
                indices_var = indices_var.cuda()
            ...
            for mu, flat_decision_p in zip(mu_e, flat_decision_p_e):
                mu = torch.mul(
                    mu,
                    torch.gather(flat_decision_p, 0, indices_var).view(
                        n_batch, self._nleaf
                    ),
                )
    ...
    def compute_py_x(self, mu_e, leaf_p_e):
        ...
        for i in range(len(mu_e)):
            py_x_tree = (
                mu_e[i]
                .unsqueeze(2)
                .repeat(1, 1, self._nlabel)
                .mul(leaf_p_e[i].unsqueeze(0).repeat(n_batch, 1, 1))
                .mean(1)
            )
    ...
    def forward(self, x):
        ...
        for i in range(len(self.treelayers)):
            ...
            leaf_p = F.softmax(self.pi_e[i], dim=1)
...
model = DeepNeuralDecisionForest(
